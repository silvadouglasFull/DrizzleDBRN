...
interface UsersRepositoryInterface {
    ...
    select<K extends keyof UserType>(fields: K[]): Promise<Pick<UserType, K>[]>
...
class UsersRepository implements UsersRepositoryInterface {
    ...
    async select<K extends keyof UserType>(fields: K[]): Promise<Pick<UserType, K>[]> {
        const result = await this.select_.select(this.createFieldsObject.createObject(fields, user)).from(user)
        return result as Pick<UserType, K>[];
    }
    async selectDistinct<K extends keyof UserType>(fields: K[]): Promise<Pick<UserType, K>[]> {
        const result = await this.selectDistinct_.selectDistinct(this.createFieldsObject.createObject(fields, user)).from(user);
        return result as Pick<UserType, K>[];
    }
}
